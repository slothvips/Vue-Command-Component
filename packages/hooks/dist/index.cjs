...
const useConsumersManager = () => {
	return {
		activeConsumers: __vue_cmd_core.activeConsumers,
		hideAll: () => {
			__vue_cmd_core.activeConsumers.forEach((consumer) => consumer.hide());
		},
		showAll: () => {
			__vue_cmd_core.activeConsumers.forEach((consumer) => consumer.show());
		},
		toggleAll: () => {
			__vue_cmd_core.activeConsumers.forEach((consumer) => {
		...
		destroyAllWithResolve: () => {
			const promises = [...__vue_cmd_core.activeConsumers].map((consumer) => {
...
const useDestroyAllOnRouteChange = () => {
	...
	return (0, vue.watch)(() => route.path, () => destroyAll(), { immediate: true });
};
/**
...
const useRawCommand = (useConfig) => {
	...
	return (vnode, config = {}) => {
		...
		const visible = (0, vue.ref)((_finalConfig$immediat = finalConfig.immediate) !== null && _finalConfig$immediat !== void 0 ? _finalConfig$immediat : true);
		finalConfig.visible = visible;
		const consumer = (0, __vue_cmd_core.commandProviderWithRender)(instance, (0, vue.h)((0, vue.defineComponent)({ setup() {
			finalConfig.displayDirective === "show" && (0, vue.watch)(() => visible.value, () => {
				(0, vue.nextTick)().then(() => {
...
//#endregion
exports.useConsumersManager = useConsumersManager;
exports.useDestroyAllOnRouteChange = useDestroyAllOnRouteChange;
exports.useRawCommand = useRawCommand;