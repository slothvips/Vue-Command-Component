 ...
 */
export const useRawCommand = (
  useConfig: IUseConfig<{
  ...
  return (vnode: VNode, config: ICoreConfig = {} as ICoreConfig) => {
    ...
    const consumer = commandProviderWithRender(
      ...
      h(
        defineComponent({
          setup() {
            finalConfig.displayDirective === "show" &&
              watch(
                () => visible.value,
                () => {
                  nextTick().then(() => {
                    const collect = (targetVnode: VNode): any[] => {
                      if (targetVnode.shapeFlag === 1) {
                        ...
                        return (targetVnode.children as VNode[]).map(
                          (child: VNode) => {
                    ...
                    els.forEach((el) => {
                      visible.value
                        ? finalConfig.onShow?.(el, consumer)
                        : finalConfig.onHide?.(el, consumer);
                    });
                  });
                },
                {
